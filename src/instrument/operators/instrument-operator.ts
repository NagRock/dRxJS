...
import {rx} from '../rx';
import {from, identity, isObservable, noop, Observable, Observer} from 'instrumented-rxjs';
import {Receiver, Sender} from '../types';
import {RxOperator} from './types';

export const instrumentOperator = (
  instrumentArgs: (args: any[], instrument) => any[] = identity,
  onReceived: (inputKey: number | string, notificationId: number) => void = noop,
  onSend: (notificationId: number) => void = noop,
) =>
